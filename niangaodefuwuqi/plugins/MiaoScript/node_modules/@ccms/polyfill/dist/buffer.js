...
Object.defineProperty(BufferPolyfill.prototype, 'parent', {
    enumerable: true,
    get: function () {
        if (!BufferPolyfill.isBuffer(this))
            return undefined;
        return this.buffer;
    }
});
...
BufferPolyfill.compare = function compare(a, b) {
    if (isInstance(a, Uint8Array))
        a = BufferPolyfill.from(a, a.offset, a.byteLength);
    if (isInstance(b, Uint8Array))
        b = BufferPolyfill.from(b, b.offset, b.byteLength);
...
BufferPolyfill.concat = function concat(list, length) {
    ...
    var buffer = BufferPolyfill.allocUnsafe(length);
    var pos = 0;
    for (i = 0; i < list.length; ++i) {
        var buf = list[i];
...
function byteLength(string, encoding) {
    ...
    if (typeof string !== 'string') {
        throw new TypeError('The "string" argument must be one of type string, Buffer, or ArrayBufferPolyfill. ' +
...
function bidirectionalIndexOf(buffer, val, byteOffset, encoding, dir) {
    ...
    if (byteOffset < 0)
        byteOffset = BufferPolyfill.length + byteOffset;
    if (byteOffset >= BufferPolyfill.length) {
        if (dir)
            return -1;
        else
            byteOffset = BufferPolyfill.length - 1;
    }
...
BufferPolyfill.prototype.write = function write(string, offset, length, encoding) {
    ...
    if (offset === undefined) {
        ...
        throw new Error('BufferPolyfill.write(string, encoding, offset[, length]) is no longer supported');
    }
...
BufferPolyfill.prototype.readBigUInt64LE = defineBigIntMethod(function readBigUInt64LE(offset) {
    offset = offset >>> 0;
...
BufferPolyfill.prototype.readBigUInt64BE = defineBigIntMethod(function readBigUInt64BE(offset) {
    offset = offset >>> 0;
    validateNumber(offset, 'offset');
    var first = this[offset];
    var last = this[offset + 7];
...
BufferPolyfill.prototype.readBigInt64LE = defineBigIntMethod(function readBigInt64LE(offset) {
    offset = offset >>> 0;
    validateNumber(offset, 'offset');
    var first = this[offset];
    var last = this[offset + 7];
...
BufferPolyfill.prototype.readBigInt64BE = defineBigIntMethod(function readBigInt64BE(offset) {
    offset = offset >>> 0;
    validateNumber(offset, 'offset');
    var first = this[offset];
    var last = this[offset + 7];
...
BufferPolyfill.prototype.writeBigUInt64LE = defineBigIntMethod(function writeBigUInt64LE(value, offset) {
    if (offset === void 0) { offset = 0; }
    // @ts-ignore
    return wrtBigUInt64LE(this, value, offset, BigInt(0), BigInt('0xffffffffffffffff'));
});
BufferPolyfill.prototype.writeBigUInt64BE = defineBigIntMethod(function writeBigUInt64BE(value, offset) {
    if (offset === void 0) { offset = 0; }
    // @ts-ignore
...
BufferPolyfill.prototype.writeBigInt64LE = defineBigIntMethod(function writeBigInt64LE(value, offset) {
    if (offset === void 0) { offset = 0; }
    // @ts-ignore
    return wrtBigUInt64LE(this, value, offset, -BigInt('0x8000000000000000'), BigInt('0x7fffffffffffffff'));
});
BufferPolyfill.prototype.writeBigInt64BE = defineBigIntMethod(function writeBigInt64BE(value, offset) {
    if (offset === void 0) { offset = 0; }
    // @ts-ignore
...
BufferPolyfill.prototype.fill = function fill(val, start, end, encoding) {
    ...
    if (typeof val === 'number') {
        ...
        var bytes = BufferPolyfill.isBuffer(val)
            ? val
            // @ts-ignore
...
function E(sym, getMessage, Base) {
    errors[sym] = /** @class */ (function (_super) {
